...
const settingUpdaters: {
  ...
  itn_enabled: (value) => commands.changeItnEnabledSetting(value as boolean),
  punctuation_enabled: (value) =>
    commands.changePunctuationEnabledSetting(value as boolean),
  llm_roles: (value) => commands.updateLlmRoles(value as any),
  selected_llm_role_id: async (value) => {
    await (commands as any).setSelectedLlmRoleId(value as string | null);
  },
  llm_configs: async (value) => {
    await (commands as any).updateLlmConfigs(value as any);
  },
  selected_llm_config_id: async (value) => {
    await (commands as any).updateSelectedLlmConfigId(value as string | null);
  },
  llm_provider_id: async (value) => {
    const settings = useSettingsStore.getState().settings;
    await commands.updateLlmGlobalConfig(
      value as string | null,
      settings?.llm_api_key || null,
      settings?.llm_model || null,
    );
  },
  llm_api_key: async (value) => {
    const settings = useSettingsStore.getState().settings;
    await commands.updateLlmGlobalConfig(
      settings?.llm_provider_id || null,
      value as string | null,
      settings?.llm_model || null,
    );
  },
  llm_model: async (value) => {
    const settings = useSettingsStore.getState().settings;
    await commands.updateLlmGlobalConfig(
      settings?.llm_provider_id || null,
      settings?.llm_api_key || null,
  ...
  transcription_api_configs: async (value) => {
    await (commands as any).updateTranscriptionApiConfigs(value as any);
  },
  selected_transcription_api_config_id: async (value) => {
    await (commands as any).updateSelectedTranscriptionApiConfigId(value as string | null);
  },
  use_transcription_api: async (value) => {
    await (commands as any).updateUseTranscriptionApi(value as boolean);
  },
};
