...
interface ModelDropdownProps {
  ...
  onModelCancelDownload?: (modelId: string) => Promise<void>;
...
const ModelDropdown: React.FC<ModelDropdownProps> = ({
  ...
  const handleCancelDownload = async (e: React.MouseEvent, modelId: string) => {
    e.preventDefault();
    e.stopPropagation();

    if (onModelCancelDownload) {
      try {
        await onModelCancelDownload(modelId);
      } catch (err) {
  ...
  return (
    <div className="absolute bottom-full left-0 mb-2 w-64 bg-background border border-mid-gray/20 rounded-lg shadow-lg z-50 flex flex-col max-h-[32rem]">
      {/* Drag Handle */}
      <div className="flex justify-center py-1.5 border-b border-mid-gray/10 cursor-grab active:cursor-grabbing">
        <div className="w-8 h-1 bg-mid-gray/30 rounded-full" />
      </div>

      {/* Scrollable Content */}
      <div className="overflow-y-auto py-2 model-dropdown-scroll">
        {/* First Run Welcome */}
        {isFirstRun && (
                ...
                >
                  ...
                  {isDownloading && progress && (
                    <div className="mt-2 flex items-center gap-2">
                      ...
                      {onModelCancelDownload && (
                        <button
                          onClick={(e) => handleCancelDownload(e, model.id)}
                          className="flex-shrink-0 p-1 hover:bg-mid-gray/20 rounded transition-colors"
                          title={t("modelSelector.cancelDownload")}
                          aria-label={t("modelSelector.cancelDownload")}
                        >
                          <X className="w-3 h-3 text-text/60 hover:text-text/80" />
                        </button>
                      )}
                    </div>
                  )}
