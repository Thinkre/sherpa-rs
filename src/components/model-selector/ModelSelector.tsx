...
const ModelSelector: React.FC<ModelSelectorProps> = ({ onError }) => {
  ...
  useEffect(() => {
    ...
    // Listen for model download failures
    const downloadFailedUnlisten = listen<{
      model_id: string;
      error: string;
      ...
      setModelError(`Failed to download model: ${event.payload.error}`);
      setModelStatus("error");
      loadModels(); // Refresh models list
    });
  ...
  const loadModels = async () => {
    try {
      ...
      if (result.status === "ok") {
        // Filter out unstable models that crash (low-level FFI issues)
        const UNSTABLE_MODELS = [
          'zipformer-zh-en',        // Transducer - crashes
          'fire-red-asr',           // FireRedAsr - crashes
          'seaco-paraformer',       // SeacoParaformer - crashes
          'streaming-paraformer',   // StreamingParaformer - may crash
        ];

        const stableModels = result.data.filter(
          (model: ModelInfo) => !UNSTABLE_MODELS.includes(model.id)
        );

        setModels(stableModels);
      }
    } catch (err) {
  ...
  const handleModelCancelDownload = async (modelId: string) => {
    try {
      ...
      <div className="relative" ref={dropdownRef}>
        ...
        {showModelDropdown && (
          <ModelDropdown
            ...
            onModelCancelDownload={handleModelCancelDownload}
