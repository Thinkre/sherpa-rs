...
/// Note: We do NOT restore the original clipboard content after pasting, as this can cause
/// applications to read the old clipboard content again and paste it, resulting in duplicate content.
/// Instead, we leave the new text in the clipboard, which is the expected behavior.
fn paste_via_clipboard(
    ...
    let _clipboard_content = clipboard.read_text().unwrap_or_default();

    debug!("Paste via clipboard: original clipboard content saved (will NOT restore after paste)");

    // Write text to clipboard first
    debug!(
        "Writing text to clipboard (length: {}): '{}'",
        text.len(),
    ...
    let verify_content = clipboard.read_text().unwrap_or_default();
    if verify_content != text {
        warn!(
            "Clipboard write verification failed. Expected: '{}', Got: '{}'",
            if text.chars().count() > 50 {
                format!("{}...", text.chars().take(50).collect::<String>())
            } else {
                text.to_string()
            },
            if verify_content.chars().count() > 50 {
                format!("{}...", verify_content.chars().take(50).collect::<String>())
            } else {
                verify_content
            }
        );
    } else {
        debug!("Clipboard write verified successfully");
    }

    std::thread::sleep(std::time::Duration::from_millis(50));

    // Send paste key combo
    debug!("Sending paste key combo: {:?}", paste_method);
    #[cfg(target_os = "linux")]
    ...
    // DO NOT restore original clipboard content after pasting
    // Reason: Restoring the clipboard too early can cause applications to read the old
    // clipboard content again and paste it, resulting in duplicate/incorrect content.
    // The new text will remain in the clipboard, which is the expected behavior.
    debug!("Paste completed. Clipboard NOT restored to avoid duplicate pasting.");
