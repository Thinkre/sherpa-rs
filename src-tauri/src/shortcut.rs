...
#[tauri::command]
#[specta::specta]
pub fn update_llm_roles(app: AppHandle, roles: Vec<settings::LLMRole>) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
...
#[tauri::command]
#[specta::specta]
pub fn set_selected_llm_role_id(app: AppHandle, role_id: Option<String>) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.selected_llm_role_id = role_id.clone();
    
    // If selecting a role, enable LLM processing
    if role_id.is_some() && !settings.llm_enabled {
...
#[tauri::command]
#[specta::specta]
pub fn update_llm_global_config(
    app: AppHandle,
...
#[tauri::command]
#[specta::specta]
pub fn update_llm_configs(app: AppHandle, configs: Vec<settings::LLMConfig>) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
...
#[tauri::command]
#[specta::specta]
pub fn update_selected_llm_config_id(
    app: AppHandle,
    config_id: Option<String>,
) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.selected_llm_config_id = config_id;
    settings::write_settings(&app, settings);
    Ok(())
}
...
#[tauri::command]
#[specta::specta]
pub fn update_transcription_api_configs(
    app: AppHandle,
    configs: Vec<settings::TranscriptionApiConfig>,
) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.transcription_api_configs = configs;
    settings::write_settings(&app, settings);
    Ok(())
}
...
#[tauri::command]
#[specta::specta]
pub fn update_selected_transcription_api_config_id(
    app: AppHandle,
    config_id: Option<String>,
) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.selected_transcription_api_config_id = config_id;
    settings::write_settings(&app, settings);
    Ok(())
}
...
#[tauri::command]
#[specta::specta]
pub fn update_use_transcription_api(app: AppHandle, enabled: bool) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.use_transcription_api = enabled;
    settings::write_settings(&app, settings);
    Ok(())
}
...
#[tauri::command]
#[specta::specta]
pub async fn test_llm_config(app: AppHandle, config_id: String) -> Result<String, String> {
    let settings = settings::get_settings(&app);
    ...
    let (provider, custom_api_url) = if config.provider_id == "custom" {
        // For custom provider, use the user-provided API URL as base_url
        // This matches the logic in actions.rs
        let custom_provider = settings::PostProcessProvider {
            id: "custom".to_string(),
        ...
        // For custom provider, pass the api_url as custom_api_url to ensure it's used
        (custom_provider, config.api_url.clone())
    } else {
        // Find the provider
        let provider = settings
            .post_process_providers
            ...
            .clone();
        // For non-custom providers, use config.api_url if it differs from provider's default base_url
        let custom_url = if let Some(config_url) = &config.api_url {
            if config_url.trim_end_matches('/') != provider.base_url.trim_end_matches('/') {
                Some(config_url.clone())
            } else {
                None
            }
        } else {
    ...
    // Test the connection
    match crate::llm_client::test_llm_config_with_url(
        &provider,
...
pub fn change_app_language_setting(app: AppHandle, language: String) -> Result<(), String> {
    ...
    // Refresh the app menu with the new language
    #[cfg(target_os = "macos")]
    {
        if let Err(e) = crate::app_menu::update_app_menu(&app) {
