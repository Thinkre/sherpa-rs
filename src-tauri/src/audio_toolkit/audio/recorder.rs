...
pub struct AudioRecorder {
    ...
    streaming_cb: Option<Arc<dyn Fn(&[f32]) + Send + Sync + 'static>>, // Callback for streaming recognition
}
...
impl AudioRecorder {
    ...
    pub fn open(&mut self, device: Option<Device>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let worker = std::thread::spawn(move || {
            ...
            run_consumer(sample_rate, vad, sample_rx, cmd_rx, level_cb, streaming_cb);
            // #region agent log
            let log_msg = format!(
                r#"{{"id":"log_{}_{}","timestamp":{},"location":"recorder.rs:132","message":"run_consumer returned, stream should be dropped","data":{{}},"sessionId":"debug-session","runId":"run1","hypothesisId":"E"}}"#,
        ...
        f32: cpal::FromSample<T>,
    {
        ...
        let stream_cb = move |data: &[T], _: &cpal::InputCallbackInfo| {
            ...
            // #region agent log
            let send_result = sample_tx.send(output_buffer.clone());
            if let Err(e) = send_result {
                let log_msg = format!(
                    r#"{{"id":"log_{}_{}","timestamp":{},"location":"recorder.rs:201","message":"Failed to send samples","data":{{"error":"{:?}","buffer_len":{}}},"sessionId":"debug-session","runId":"run1","hypothesisId":"A"}}"#,
                    std::time::SystemTime::now()
                        .duration_since(std::time::UNIX_EPOCH)
                        .unwrap()
                        .as_nanos(),
                ...
                let log_msg = format!(
                    r#"{{"id":"log_{}_{}","timestamp":{},"location":"recorder.rs:201","message":"Successfully sent samples","data":{{"buffer_len":{}}},"sessionId":"debug-session","runId":"run1","hypothesisId":"A"}}"#,
                    std::time::SystemTime::now()
                        .duration_since(std::time::UNIX_EPOCH)
                        .unwrap()
                        .as_nanos(),
...
fn run_consumer(
    ...
    level_cb: Option<Arc<dyn Fn(Vec<f32>) + Send + Sync + 'static>>,
    streaming_cb: Option<Arc<dyn Fn(&[f32]) + Send + Sync + 'static>>,
) {
    ...
    loop {
        ...
        let raw = match sample_rx.recv() {
            Ok(s) => {
                // #region agent log
                let log_msg = format!(
                    r#"{{"id":"log_{}_{}","timestamp":{},"location":"recorder.rs:303","message":"Received samples","data":{{"sample_len":{},"recording":{}}},"sessionId":"debug-session","runId":"run1","hypothesisId":"B"}}"#,
                    std::time::SystemTime::now()
                        .duration_since(std::time::UNIX_EPOCH)
                        .unwrap()
                        .as_nanos(),
            ...
            Err(e) => {
                // #region agent log
                let log_msg = format!(
                    r#"{{"id":"log_{}_{}","timestamp":{},"location":"recorder.rs:305","message":"Channel closed, breaking loop","data":{{"error":"{:?}"}},"sessionId":"debug-session","runId":"run1","hypothesisId":"C"}}"#,
                    std::time::SystemTime::now()
                        .duration_since(std::time::UNIX_EPOCH)
                        .unwrap()
                        .as_nanos(),
        ...
        frame_resampler.push(&raw, &mut |frame: &[f32]| {
            ...
            // Call streaming callback if recording and callback is set
            if recording {
                if let Some(ref cb) = streaming_cb {
                    cb(frame);
                }
            }
        });
