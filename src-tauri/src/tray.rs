...
pub fn update_tray_menu(app: &AppHandle, state: &TrayIconState, locale: Option<&str>) {
    ...
    // Format: 🎤 KeVoiceInput v0.1.0 (icon + name + version)
    let version_label = if cfg!(debug_assertions) {
        format!("🎤 KeVoiceInput v0.1.0 (Dev)")
    } else {
        format!("🎤 KeVoiceInput v0.1.0")
    };
    let version_i = MenuItem::with_id(app, "version", &version_label, false, None::<&str>)
        .expect("failed to create version item");
    let show_home_i = MenuItem::with_id(app, "show_home", &strings.show_home, true, None::<&str>)
    ...
    let add_hotword_i =
        MenuItem::with_id(app, "add_hotword", &strings.add_hotword, true, None::<&str>)
            .expect("failed to create add hotword item");
...
mod tests {
    ...
    fn build_entry(transcription: &str, post_processed: Option<&str>) -> HistoryEntry {
        HistoryEntry {
            id: 1,
            file_name: "kevoiceinput-1.wav".to_string(),
            timestamp: 0,
            saved: false,
