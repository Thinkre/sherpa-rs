...
#[tauri::command]
#[specta::specta]
pub async fn reload_models_config(
    model_manager: State<'_, Arc<ModelManager>>,
) -> Result<(), String> {
    model_manager
...
pub async fn get_recommended_first_model() -> Result<String, String> {
    // Recommend Whisper Small model for first-time users - fast and accurate
    Ok("small".to_string())
}

#[tauri::command]
#[specta::specta]
pub async fn import_local_model_folder(
    model_manager: State<'_, Arc<ModelManager>>,
    source_path: String,
    ...
    model_manager
        .import_local_model_folder(path)
        .map_err(|e| e.to_string())
}
